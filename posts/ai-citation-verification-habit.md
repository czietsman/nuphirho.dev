...
subtitle: "Five ways AI citations fail, why expert peer review missed all of them, and the verification habit that catches what fluency cannot"
tags: [research, ai-assisted-development]
d1_provenance: [d1-0316, d1-0317, d1-0318, d1-0319, d1-0320, d1-0321]
publish_date: 2026-08-24
format: article
cover_image: ai-citation-verification-habit.jpg
cover_image_prompt: |
  A single small knotted thread at the top of the image, drawn in minimal hand-drawn cartoonist line work on aged paper, running down and looping past one plain document to attach instead to a second, identical-looking document beside it. A single warm amber glow falls on the point where the thread attaches to the second document. Aged paper texture, cartoonist, muted slate and off-white, with the glow as the only warmth in the image. Still, institutional. No people, no screens, no words or letters anywhere in the image, no colour accents beyond the glow.
cover_post: |
  AI citation fabrication is not an edge case. A February 2026 arXiv study found one hundred fabricated citations across fifty-three NeurIPS 2025 papers, each one already through three to five expert peer reviewers. Reviewers in their own field, checking for exactly this, missed it.

  The habit I have built since noticing this in my own work: treat AI citation output as a research direction, not a reference. Fetch the primary source. Read what it actually says. Confirm the metadata matches, because a resolving identifier is not evidence on its own, sometimes it resolves to a real paper about something else entirely.

  New piece on the five failure modes and the verification habit that catches them.
notes: "Converted to format: article, 23 August 2026, per Director instruction: the body runs to roughly 6,400 characters, well over the LinkedIn post limit. Added subtitle, cover_image, cover_image_prompt (following the current minimalist convention: single visual metaphor, aged paper, cartoonist, single warm glow, no rendered text, per meta/artifacts/cover-image-style-guide.md), and cover_post as a teaser under the 1,300-character template limit. Body reviewed against meta/artifacts/voice-principles-blogger.md and found clean: no em dashes or double dashes, British spelling consistent, no significance-announcing phrases remaining after the 'worth holding onto' fix applied in the previous session. Body itself unchanged beyond that earlier fix. Added the arXiv identifier (arXiv:2602.05930, February 2026) at first mention of Ansari's name, 23 August 2026, per Director instruction, plain text per this blog's existing citation convention (parenthetical arXiv ID, no hyperlink), matching how 01-correlated-error-v2.md, 02-cynefin-domain-transition-v2.md, and others cite papers inline. D1-confirmed citation: Mohammad Samar Ansari (2026), 'Compound Deception in Elite Peer Review: A Failure Mode Taxonomy of 100 Fabricated Citations at NeurIPS 2025', arXiv:2602.05930v1 [cs.DL, cs.AI], submitted 5 February 2026, DOI 10.48550/arXiv.2602.05930; Director fetched the arXiv record directly, per research-notes/so-synthesis-225-p12-citation-fabrication-taxonomy-2july2026.md. Added the missing d1_provenance field 10 September 2026, naming d1-0316 to d1-0321, after the Science Officer found the post's Ansari citation had no register entry despite being cited as D1-confirmed; all six are now confirmed against the primary PDF the Director supplied. Corrected the same day: the description of the residual three per cent (placeholder and semantic hallucination) was wrong, and for semantic hallucination close to the inverse of the paper's own definition. The paper defines semantic hallucination as an invented title that fits the domain but corresponds to no real paper, not a real paper cited for a claim it does not support; placeholder hallucination is unfilled template fields (an author placeholder, an incomplete identifier), not a second name for the same thing. Rewrote the sentence to match section 2.2 of the source. Neither category the post previously described, a real source misapplied to an unsupported claim, is in this taxonomy at all."
linkedin_url:
---
...
That figure removes the story in which this is a problem only for careless users who do not know what they are dealing with. Domain experts, reviewing work in their own field, with all the motivation to check that peer review provides, missed fabricated citations at scale.

The taxonomy Mohammad Samar Ansari (arXiv:2602.05930, February 2026) developed for his analysis breaks the failure modes into five types. Total fabrication, where the cited paper simply does not exist, accounts for sixty-six per cent of cases. Partial attribute corruption, where the paper exists but key metadata (author, year, title, journal) is wrong, accounts for another twenty-seven per cent. Identifier hijacking, the most structurally interesting failure mode, accounts for four per cent: the citation carries a valid identifier that resolves to a real paper, but the metadata fabricated around it does not match that paper. The identifier looks verifiable. It resolves. The paper on the other end is not the one being cited. The remaining three per cent covers placeholder hallucinations, citations still carrying unfilled template fields such as an author placeholder or an incomplete identifier, and semantic hallucinations, a plausible-sounding title invented to fit the topic that does not correspond to any real paper.
