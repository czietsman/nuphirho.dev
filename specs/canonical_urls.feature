Feature: Canonical URL management
  As the blog author
  I want all cross-posted content to reference the canonical URL on blog.nuphirho.dev
  So that SEO authority builds on the blog subdomain

  Scenario: Canonical URL is set on Hashnode publication
    Given a post with slug "setting-up-the-blog" is published to Hashnode
    Then the canonical URL is "https://blog.nuphirho.dev/setting-up-the-blog"
